...
Run example
-----------
python train_kth.py --data_root ~/data/kth --epochs 50 --batch_size 64 \
    --seq_len 20 --height 64 --lr 1e‑3 --device cuda
"""

import argparse, random, re, urllib.request, zipfile, math
...
class KTHVideoClips(Dataset):
    ...
    def _apply_velocity_shift(self, frame, vy, vx):
        ...
        #    we reshape kernel to match C channels by grouping in groups=C
        # shift_kernel = shift_kernel.repeat(C, 1, 1, 1)
...
class FakeCovariantRNN(GalileanCovariantRNN):
    def __init__(self, input_channels, hidden_channels, kernel_size, num_layers=1, v_list=[(0,0)]):
        super(FakeCovariantRNN, self).__init__(input_channels, hidden_channels, kernel_size, num_layers, v_list)
...
class CovariantPatternDetectorRNN(nn.Module):
    def __init__(self, input_size, hidden_size, hidden_channels, num_patterns, v_list=[(0,0)], num_layers=1, use_fake_rnn=False):
        super(CovariantPatternDetectorRNN, self).__init__()
        self.hidden_size = hidden_size
        self.hidden_channels = hidden_channels
        rnn_class = FakeCovariantRNN if use_fake_rnn else GalileanCovariantRNN
...
def main():
    ...
    parser.add_argument("--vx_range", type=int, default=0, help="Range of x-velocities to consider (-vx_range to vx_range)")
    parser.add_argument("--vy_range", type=int, default=0, help="Range of y-velocities to consider (-vy_range to vy_range)")
    parser.add_argument("--vx_data_range", type=int, default=0, help="Range of x-velocities for data augmentation (-vx_data_range to vx_data_range)")
    parser.add_argument("--vy_data_range", type=int, default=0, help="Range of y-velocities for data augmentation (-vy_data_range to vy_data_range)")
    ...
    parser.add_argument("--use_fake_rnn", action="store_true", help="Use learned kernels instead of shift kernels")
    parser.add_argument("--channel_multiplier", type=int, default=1, help="Channel multiplier")
    ...
    parser.add_argument("--model", type=str, default="galilean", choices=["galilean", "3dcnn"], help="Model to use")
    ...
    parser.add_argument("--vx_gen_test_range", type=int, default=0, help="Range of x-velocities for velocity generalization test sets (-vx_gen_test_range to vx_gen_test_range)")
    ...
    if args.vx_gen_test:
        ...
        print(f"Generating {args.vx_gen_test_range} velocity generalization test sets …")

        for vx_gen_test_range in range(1, args.vx_gen_test_range + 1):
            v_gen_test_datasets[vx_gen_test_range] = KTHVideoClips(
                Path(args.data_root), 
                seq_len=16, 
                height=args.height, 
                width=args.width, 
                step=2, 
                split='test',
                vx_data_range=vx_gen_test_range,
                vy_data_range=0,
                split_method=args.split_method
            )

            v_gen_test_loaders[vx_gen_test_range] = DataLoader(         
                v_gen_test_datasets[vx_gen_test_range], 
                batch_size=args.batch_size, 
                shuffle=False, 
                num_workers=args.num_workers,
                pin_memory=True,
    ...
    if args.model == "galilean":
        model = CovariantPatternDetectorRNN(
    ...
    for epoch in range(args.epochs):
        ...
        if args.vx_gen_test:
            ...
            for vx_gen_test_range in range(1, args.vx_gen_test_range + 1):
                v_gen_test_loss, v_gen_test_acc = _eval(model, v_gen_test_loaders[vx_gen_test_range], criterion, args.device)
                wandb.log({
                    f"v_gen_{vx_gen_test_range}_test_loss": v_gen_test_loss,
                    f"v_gen_{vx_gen_test_range}_test_acc": v_gen_test_acc
                })
                print(f"V-Gen {vx_gen_test_range} Test Loss: {v_gen_test_loss:.4f}, V-Gen {vx_gen_test_range} Test Acc: {v_gen_test_acc:.4f}")
                v_gen_test_losses[vx_gen_test_range] = v_gen_test_loss
                v_gen_test_accs[vx_gen_test_range] = v_gen_test_acc
        ...
        if val_acc > best_val_acc:
            ...
            if args.vx_gen_test:
                best_v_gen_test_accs = {}
                for vx_gen_test_range in range(1, args.vx_gen_test_range + 1):
                    best_v_gen_test_accs[vx_gen_test_range] = v_gen_test_accs[vx_gen_test_range]
    ...
    if args.wandb:
        ...
        if args.vx_gen_test:
            for vx_gen_test_range in range(1, args.vx_gen_test_range + 1):
                wandb.log({
                    f"best_v_gen_{vx_gen_test_range}_test_acc": best_v_gen_test_accs[vx_gen_test_range]
                })
